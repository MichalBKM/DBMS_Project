...
def query_2():
    ...
    query_2_text = """SELECT m.title, m.release_year, p.person_name AS director, m.overview, m.vote_average
                 FROM movie m
...
def query_4():
    ...
    query_4_text = """SELECT COUNT(m.movie_id) as movie_count, p.person_name AS name, p.person_id, p.birthday
...
def query_5():
    ...
    query_5_text = """SELECT m.title, m.release_year, m.overview, m.vote_average, m.popularity
                        FROM movie m
...
#  EXTRA QUERIES  #
###################
#6 EXTRA QUERY: "Most popular movies of a director" - for a given director, output their most popular movies
def query_6():
    print("We will now present the most popular movies for a given director\n")
    input_6 = input("Enter your favourite director's name to see their most popular movies! (e.g, 'Stanley Kubrick') \n")
...
#7 EXTRA QUERY: "Most popular movies of an actor" - for a given actor, output their most popular movies
def query_7():
    print("We will now present the most popular movies for a given actor\n")
    input_7 = input("Enter your favourite actor's name to see what movie you see! (e.g, 'Tom Hanks'): \n")
...
def query_8():
    ...
    input_8 = input("Enter an actor's name to see the genres they have appeared in the most (e.g, 'Maryl Streep'): \n")
